...
impl ConfigValidator {
    ...
    fn validate_policy(policy: &PolicyConfig) -> ConfigResult<()> {
        match policy {
            PolicyConfig::Random | PolicyConfig::RoundRobin | PolicyConfig::LeastConnections => {
    ...
    fn validate_compatibility(config: &RouterConfig) -> ConfigResult<()> {
        ...
        if !has_service_discovery {
            // Power-of-two ideally wants >=2 workers, but we allow boot-time empty
            // bootstrap when workers are added dynamically (persisted state / controller).
            // Runtime selection logic already handles the <2 workers case (falls back to
            // the single worker or returns None).
