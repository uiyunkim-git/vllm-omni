...
pub fn start_health_checker(
    ...
    let handle = tokio::spawn(async move {
        ...
        loop {
            ...
            // Note: periodic "drift prevention" reset used to live here, blanket-zeroing
            // every worker's load_counter when max appeared low. With accurate
            // increment/decrement tracking in route_typed_request + send_typed_request,
            // that reset corrupts the gauge under heavy traffic (it would zero counters
            // mid-flight, then the gauge regrew from 0 while real in-flight stayed high).
            // Removed.
